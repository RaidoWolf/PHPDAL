        ...
        if (func_num_args() > 0) {
            if (func_num_args() == 1 && is_array(func_get_arg(0))) {
                foreach (func_get_arg(0) as $columnKey => $column) {
                    if (is_subclass_of($column, 'SchemaViewColumn')) {
                        $this->columns[] = $column;
                    } else {
                        throw new DatabaseException(
                            __METHOD__.'(): Column at ['.$columnKey.'] was not an instance of class SchemaViewColumn.',
                ...
                foreach (func_get_args() as $columnKey => $column) {
                    if (is_subclass_of($column, 'SchemaViewColumn')) {
                        ...
                        throw new DatabaseException(
                            __METHOD__.'(): Column at argument position ['.$columnKey.'] was not an instance of class SchemaViewColumn.',
                            DatabaseException::EXCEPTION_INPUT_INVALID_TYPE,
                            $this
                        );
                    }
